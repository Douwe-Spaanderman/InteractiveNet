...
class DynUNet(nn.Module):
    ...
    def __init__(
        ...
        self.input_block_1 = self.get_input_block()
        self.input_block_2 = self.get_input_block()
        self.downsamples_1 = self.get_downsamples()
        self.downsamples_2 = self.get_downsamples()
        self.bottleneck_1 = self.get_bottleneck()
        self.bottleneck_2 = self.get_bottleneck()
        self.upsamples = self.get_upsamples()
        self.output_block = self.get_final_output_block(0)
        self.deep_supervision = deep_supervision
        self.deep_supr_num = deep_supr_num
        # initialize the typed list of supervision head outputs so that Torchscript can recognize what's going on
        self.heads_1: List[torch.Tensor] = [torch.rand(1)] * self.deep_supr_num
        self.heads_2: List[torch.Tensor] = [torch.rand(1)] * self.deep_supr_num
        
        if self.deep_supervision:
            self.deep_supervision_heads_1 = self.get_deep_supervision_heads()
            self.deep_supervision_heads_2 = self.get_deep_supervision_heads()
            self.check_deep_supr_num()

        self.apply(self.initialize_weights)
        self.check_kernel_stride()

        def create_skips_1(index, downsamples, upsamples, bottleneck, superheads=None):
            """
            ...
            if superheads is None:
                next_layer = create_skips_1(1 + index, 
                                          downsamples[1:],
                                          upsamples[1:], 
                                          bottleneck,)
                return DynUNetSkipLayer(index, 
        ...
        def create_skips_2(index, downsamples, upsamples, bottleneck, superheads=None):
            """
            Construct the UNet topology as a sequence of skip layers terminating with the bottleneck layer. This is
            done recursively from the top down since a recursive nn.Module subclass is being used to be compatible
            with Torchscript. Initially the length of `downsamples` will be one more than that of `superheads`
            ...
            if superheads is None:
                next_layer = create_skips_2(1 + index, 
                                          downsamples[1:],
                                          upsamples[1:], 
                                          bottleneck,)
                return DynUNetSkipLayer(index, 
        ...
        if not self.deep_supervision:
            self.skip_layers_1 = create_skips_1(
                0, 
                [self.input_block_1] + list(self.downsamples_1), 
                self.upsamples[::-1], 
                self.bottleneck_1,
            )
            self.skip_layers_2 = create_skips_2(
                0, 
                [self.input_block_2] + list(self.downsamples_2), 
                self.upsamples[::-1], 
                self.bottleneck_2,
            )
        else:
            self.skip_layers_1 = create_skips_1(
                ...
                self.upsamples[::-1],
                self.bottleneck_1,
                superheads=self.deep_supervision_heads_1,
            )
            self.skip_layers_2 = create_skips_2(
                0,
                [self.input_block_2] + list(self.downsamples_2),
                self.upsamples[::-1],
                self.bottleneck_2,
                superheads=self.deep_supervision_heads_2,
            )
    ...
    def forward(self, x):
        ...
        out1 = self.skip_layers_1(x_1)
        out2 = self.skip_layers_2(x_2)
        out = torch.cat([out1, out2], dim=1)
        out = self.output_block(out)
        if self.training and self.deep_supervision:
            out_all = torch.zeros(out.shape[0], len(self.heads_1) + len(self.heads_2) + 1, *out.shape[1:], device=out.device, dtype=out.dtype)
            out_all[:, 0] = out
            for idx, feature_map in enumerate(self.heads_1):
                out_all[:, idx + 1] = interpolate(feature_map, out.shape[2:])
                
            for idx, feature_map in enumerate(self.heads_2):
                out_all[:, idx + 1 + len(self.heads_2)] = interpolate(feature_map, out.shape[2:])
    ...
    def get_final_output_block(self, idx: int):
        return UnetOutBlock(self.spatial_dims, self.filters[idx]*2, self.out_channels, dropout=self.dropout)
