...
def read_prediction(
    ...
    raw_data, labels = read_nifti(
        data=data, raw_path=raw_path, rename_image="image_raw"
    )
    ...
    for idx, run in runs.iterrows():
        ...
        if run["tags.Mode"] == "testing":
            ...
            if weights.is_dir():
                ...
                raise ValueError(
                    "No weights are available to ensemble, please use predict with -w or --weights to save outputs as weights"
                )

            postprocessing.append(
                read_metadata(
                    experiment / "postprocessing.json",
...
def main():
    parser = argparse.ArgumentParser(description="Ensembling of predicted weights")
    ...
    outputs, postprocessing, labels = read_prediction(
        data=data, raw_path=raw, task=args.task, results=results
    )
    ensemble(
        outputs=outputs,
        metadata=metadata,
